...
function validateEnvironment() {
  ...
  if (!process.env.QINGSHI_GITHUB_TOKEN) {
    console.error('Missing QINGSHI_GITHUB_TOKEN environment variable')
    ShellOps.exit(1)
  }
}
...
function generateRelease(prereleaseId) {
  ...
  fs.writeFileSync('.qingshi-release-info.json', JSON.stringify(releaseInfo, null, 4))
}
...
function publishReleaseToGitHub(releaseInfo) {
  const repoParts = releaseInfo.repository.split('/'),
    gh = new GitHub({ token: process.env.QINGSHI_GITHUB_TOKEN }),
...
function publishRelease() {
  validateSetup()
  validateEnvironment()
  const releaseInfo = JSON.parse(fs.readFileSync('.qingshi-release-info.json', 'utf8'))
